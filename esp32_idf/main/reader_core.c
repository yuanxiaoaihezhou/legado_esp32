...
size_t reader_paginate_chapter(
    ...
    size_t page_count = 0;
    size_t offset = 0;
    while (offset < text_len && page_count < max_pages) {
        size_t remaining = text_len - offset;
        size_t copy_len = remaining > bytes_per_page ? bytes_per_page : remaining;

        out_pages[page_count].chapter_id = chapter_id;
        out_pages[page_count].page_index = (int)page_count;
        out_pages[page_count].is_last_page = (offset + copy_len >= text_len);
        memcpy(out_pages[page_count].text, chapter_text + offset, copy_len);
        out_pages[page_count].text[copy_len] = '\0';

        offset += copy_len;
        page_count++;
    }

    if (offset < text_len && page_count > 0) {
        out_pages[page_count - 1].is_last_page = true;
    }
    return page_count;
}
